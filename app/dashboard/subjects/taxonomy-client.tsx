...
// passages), shared across all colleges. Curating this structure needs
// exam.subject.manage (the page gates access). Question authoring is a separate
// page (questions-client.tsx / /dashboard/questions).
import { useCallback, useEffect, useState } from "react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Card, CardContent } from "@/components/ui/card";
import type { Chapter, Passage, Subject } from "@/lib/exam-query";
...
export function TaxonomyClient() {
  ...
  useEffect(() => { loadChapters(subjectId); loadPassages(subjectId); }, [subjectId, loadChapters, loadPassages]);
        ...
        <CardContent className="grid gap-4 pt-6 sm:grid-cols-2">
          ...
          <div className="grid gap-1.5">
            ...
            <div className="flex gap-2">
              ...
              />
              <Button type="button" onClick={createSubject} disabled={!newSubject.trim()}>Add</Button>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <Card>
            <CardContent className="grid gap-4 pt-6">
                ...
                <ol className="columns-1 gap-x-8 sm:columns-2 lg:columns-3">
                  {chapters.map((c, i) => (
                    <li key={c.id} className="flex items-baseline gap-2 break-inside-avoid pb-1.5 text-sm">
                      <span className="text-muted-foreground w-6 shrink-0 text-right tabular-nums">{i + 1}.</span>
              ...
              <div className="flex max-w-md gap-2">
                ...
                />
                <Button type="button" onClick={createChapter} disabled={!newChapter.trim()}>Add chapter</Button>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="grid gap-4 pt-6">
              ...
              <div className="grid max-w-2xl gap-2">
                ...
                />
                <div>
                  <Button type="button" onClick={createPassage} disabled={!newPassageBody.trim()}>Add passage</Button>
                </div>
              </div>
            </CardContent>
          </Card>
