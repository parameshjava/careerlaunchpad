...
select public._seed_arith_q($q$Calendar$q$,'easy',$q$Which of the following is NOT a leap year?$q$,array[$q$2000$q$,$q$1600$q$,$q$2400$q$,$q$1900$q$],4,$q$A century year is a leap year only if divisible by 400. 1900/400 is not integer, so 1900 is not a leap year.$q$);
...
select public._seed_arith_q($q$Calendar$q$,'easy',$q$Which of these years is a leap year?$q$,array[$q$2100$q$,$q$2200$q$,$q$2400$q$,$q$2300$q$],3,$q$Century years are leap only if divisible by 400. 2400/400=6 (integer), so 2400 is a leap year.$q$);
