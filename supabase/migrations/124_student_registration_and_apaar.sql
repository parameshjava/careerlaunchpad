-- ============================================================================
-- 124_student_registration_and_apaar.sql
-- Capture two more student identifiers alongside roll_number (issue #54):
--   * registration_number — the University Registration / Enrollment No. issued
--     by the affiliating university at admission (stable across the degree).
--   * apaar_id            — the 12-digit APAAR / ABC ID (NEP-2020 "One Nation,
--     One Student ID"; the higher-ed ABC account number). Stored digits-only.
-- Both are OPTIONAL — not added to REQUIRED_FIELDS — and thread through the
-- intake pipeline exactly like roll_number (migrations 106/120):
--   1. columns on student_intake + student_profile.
--   2. import_student_intake() insert + on-conflict merge (rebased on 120).
--   3. merge_student_intake() intake→profile merge (rebased on 120).
-- Idempotent. (PEN — the UDISE+ school-level number — is intentionally NOT added
-- here: it is a school artifact with low relevance to a college platform.)
-- ============================================================================

-- 1) Columns -------------------------------------------------------------------
alter table public.student_intake  add column if not exists registration_number text;
alter table public.student_profile add column if not exists registration_number text;
alter table public.student_intake  add column if not exists apaar_id text;
alter table public.student_profile add column if not exists apaar_id text;

-- 2) import_student_intake — + registration_number, apaar_id -------------------
...
-- 3) merge_student_intake — + registration_number, apaar_id --------------------
